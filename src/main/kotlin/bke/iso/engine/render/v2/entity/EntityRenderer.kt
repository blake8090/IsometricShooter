package bke.iso.engine.render.v2.entity
...
import bke.iso.engine.render.v2.occlusion.Occlusion
 ...
 *
 * Consider that [EntityRenderer.sortRenderables] is of quadratic time complexity.
 * When renderables are grouped by layer, there are fewer but larger lists, as many entities share the same layer.
 * When grouped by rows, there are numerous but much smaller lists, since most entities do not share the same row.
 *
...
class EntityRenderer(
    ...
    private fun addRenderable(entity: Entity) {
        val renderable = createRenderable(entity)
...
//        }
    ...
    private fun draw(renderable: EntityRenderable, batch: PolygonSpriteBatch) {
        ...
        events.fire(EntityDrawn(entity, batch))
    }

    private fun createRenderable(entity: Entity): EntityRenderable? {
